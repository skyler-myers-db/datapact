...
class DataPactClient:
    ...
    def ensure_dashboard_exists(
        ...
        # --- 1. THOROUGH CLEANUP ---
        # Delete the old dashboard and all its associated components (widgets, visualizations, queries)
        # to prevent orphaned assets and ensure a clean slate.
        for d in self.w.dashboards.list(q=dashboard_name):
            if d.display_name == dashboard_name:
                logger.warning(f"Found and deleting existing dashboard (ID: {d.dashboard_id}) to recreate.")
                if d.widgets:
                    for widget in d.widgets:
                        if widget.visualization and widget.visualization.query:
                            try:
                                # Must delete widgets and visualizations before deleting the query
                                self.w.widgets.delete(widget.id)
                                self.w.visualizations.delete(widget.visualization.id)
                                self.w.queries.delete(widget.visualization.query.query_id)
                            except Exception as e:
                                logger.debug(f"Could not clean up component, may already be deleted: {e}")
                self.w.dashboards.delete(dashboard_id=d.dashboard_id)
        ...
        widget_specs = [
            ...
            {"title": "Success Rate", "sql": q("SELECT COUNT(IF(status='SUCCESS',1,NULL))*100.0/COUNT(*) as value FROM {table} WHERE run_id=(SELECT MAX(run_id) FROM {table} WHERE job_name='{job}')"), "type": "COUNTER", "options": {"counterColName": "value", "stringDecimalFormat": "0.00'%'"}, "pos": {"x": 8, "y": 0, "sizeX": 4, "sizeY": 4}},
            # Charts
            ...
            {"title": "Detailed Run History", "sql": q("SELECT task_key, status, timestamp, result_payload:count_validation.status as count_status, result_payload:row_hash_validation.status as hash_status FROM {table} WHERE job_name='{job}' ORDER BY timestamp DESC, task_key"), "type": "TABLE", "options": {}, "pos": {"x": 6, "y": 12, "sizeX": 6, "sizeY": 8}},
        ]
        ...
        for spec in widget_specs:
            query_obj = self.w.queries.create(name=f"DataPact-{job_name}-{spec['title']}", data_source_id=warehouse_id, query=spec['sql'])
            viz_obj = self.w.visualizations.create(query_id=query_obj.id, type=spec['type'], name=f"Viz-{spec['title']}", options=spec['options'])
            widgets.append(sql_service.WidgetCreate(visualization_id=viz_obj.id, position=spec['pos']))
        ...
        # Return the draft ID required by the job's dashboard task
        return dashboard.id
