...
def main() -> None:
    ...
    parser.add_argument(
        "--config", required=True, help="Path to the validation_config.yml file."
    )
    parser.add_argument(
        "--job-name",
    ...
    if args.command == "run":
        ...
        try:
            with open(args.config, "r", encoding="utf-8") as f:
                ...
                validation_config: DataPactConfig = DataPactConfig(**raw_config)
            ...
            logger.critical(f"An error occurred while parsing the YAML config: {e}")
            sys.exit(1)
