...
SELECT
  ...
  CASE WHEN rand() < 0.2 THEN 'Enterprise'
...
SELECT
  ...
  uuid() as product_id,
  CASE WHEN rand() < 0.4 THEN 'CREDIT_CARD'
  ...
  CASE WHEN rand() < 0.02 THEN 'FAILED'
       WHEN rand() < 0.05 THEN 'PENDING'
       WHEN rand() < 0.08 THEN 'REFUNDED'
       ELSE 'COMPLETED' END as transaction_status,
  ...
  CASE WHEN rand() < 0.15 THEN 'PROMO2024' ELSE NULL END as discount_code
...
SELECT 
  ...
  CONCAT(
    CASE WHEN rand() < 0.2 THEN 'Premium '
         ...
         ELSE '' END,
    CASE WHEN id % 5 = 0 THEN 'Electronics '
  ...
  CASE WHEN rand() < 0.1 THEN 'DISCONTINUED'
...
-- Simulating approved price updates (within threshold)
...
SELECT 
  ...
  CONCAT(
    CASE WHEN id % 4 = 0 THEN 'Q1_2024_'
         ...
         ELSE 'Q4_2024_' END,
    CASE WHEN rand() < 0.3 THEN 'Brand_Awareness_'
         WHEN rand() < 0.6 THEN 'Lead_Gen_'
         WHEN rand() < 0.8 THEN 'Retention_'
         ELSE 'Product_Launch_' END,
    id) as campaign_name,
  CASE WHEN id < 20 THEN 'completed'
  ...
  CASE WHEN rand() < 0.3 THEN 'DIGITAL'
       WHEN rand() < 0.6 THEN 'SOCIAL'
       WHEN rand() < 0.8 THEN 'EMAIL'
       ELSE 'TRADITIONAL' END as channel
...
SELECT 
  ...
  campaign_id,
  spend_date,
  ROUND(daily_spend * 1.001, 2) as daily_spend,  -- 0.1% FX conversion drift
  impressions,
  clicks,
  ctr,
  ROUND(cpc * 1.001, 2) as cpc,  -- Matching FX drift
  platform
...
INSERT INTO datapact_demo_catalog.source_data.salary_bands VALUES 
  ...
  ('M1', 'Manager I', 'MANAGER', 110000, 160000, 135000, 1.0),
  ('M2', 'Manager II', 'MANAGER', 140000, 200000, 170000, 1.0),
  ('SM1', 'Senior Manager', 'SENIOR_MANAGER', 180000, 250000, 215000, 1.0),
  ('DIR', 'Director', 'EXECUTIVE', 220000, 320000, 270000, 1.0),
  ('VP', 'Vice President', 'EXECUTIVE', 300000, 450000, 375000, 1.0),
  ('SVP', 'Senior Vice President', 'EXECUTIVE', 400000, 600000, 500000, 1.0),
  ('C', 'C-Suite', 'EXECUTIVE', 500000, 1000000, 750000, 1.0);
...
SELECT 
  ...
  CASE WHEN id % 30 = 0 THEN 'ADJUSTMENT'
       WHEN id % 50 = 0 THEN 'ACCRUAL'
       WHEN id % 100 = 0 THEN 'RECLASS'
       ELSE 'STANDARD' END as posting_type,
  CASE WHEN rand() < 0.95 THEN 'POSTED'
...
SELECT 
  ...
  CASE WHEN rand() < 0.3 THEN 'api-gateway'
  ...
  uuid() as trace_id,
  CASE WHEN rand() < 0.8 THEN (rand() * 1000000)::INT ELSE NULL END as user_id,
  CASE WHEN level = 'ERROR' THEN rand() * 5000 + 1000
       ELSE rand() * 500 END as latency_ms
FROM (SELECT explode(sequence(1, 10000)) as id);
...
FROM (SELECT explode(sequence(100, 599)) as id)
...
SELECT
  ...
  CASE WHEN rand() < 0.4 THEN 'DATA_ACCESS'
       WHEN rand() < 0.7 THEN 'DATA_DELETION'
       WHEN rand() < 0.9 THEN 'DATA_PORTABILITY'
       ELSE 'CONSENT_WITHDRAWAL' END as request_type,
...
FROM (
  SELECT 
    CASE id % 5
      ...
      WHEN 2 THEN 'USD/CAD'
      WHEN 3 THEN 'USD/AUD'
      ELSE 'USD/JPY' END as currency_pair,
    CASE id % 5
