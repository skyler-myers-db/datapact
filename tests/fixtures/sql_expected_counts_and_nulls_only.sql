...
source_stats AS (
  SELECT
    ...
    ,SUM(CASE WHEN `v1` IS NULL THEN 1 ELSE 0 END) AS source_nulls_v1
    ,SUM(CASE WHEN `v2` IS NULL THEN 1 ELSE 0 END) AS source_nulls_v2
  FROM `c`.`s`.`a`
)
,
target_stats AS (
  SELECT
    ...
    ,SUM(CASE WHEN `v1` IS NULL THEN 1 ELSE 0 END) AS target_nulls_v1
    ,SUM(CASE WHEN `v2` IS NULL THEN 1 ELSE 0 END) AS target_nulls_v2
  FROM `c`.`s`.`b`
)
