...
SELECT
  ...
  parse_json(to_json(struct(
    ...
    struct(
      ...
      FORMAT_NUMBER(target_nulls_v, '#,##0') AS target_nulls,FORMAT_STRING('%.2f%%', CAST(COALESCE(ABS(source_nulls_v - target_nulls_v) / NULLIF(CAST(total_compared_v AS DOUBLE), 0), 0) * 100 AS DOUBLE)) as relative_diff_percent,FORMAT_STRING('%.2f%%', CAST(0.02 * 100 AS DOUBLE)) AS tolerance_percent,
...
:run_id, :job_id, 'job_name_here', current_timestamp(), started_at, completed_at, source_catalog, source_schema, source_table, target_catalog, target_schema, target_table, result_payload FROM final_metrics_view;
