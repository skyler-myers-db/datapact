...
source_stats AS (
  SELECT
    ...
    ,TRY_CAST(SUM(`v`) AS DECIMAL(38, 6)) AS source_value_v_SUM
  FROM `c`.`s`.`a`
)
,
target_stats AS (
  SELECT
    ...
    ,TRY_CAST(SUM(`v`) AS DECIMAL(38, 6)) AS target_value_v_SUM
  FROM `c`.`s`.`b`
)
,
row_hash_metrics AS (
...
null_join_metrics AS (
  SELECT
    ...
    SUM(CASE WHEN t.`v` IS NULL THEN 1 ELSE 0 END) AS target_nulls_v  FROM `c`.`s`.`a` s
  INNER JOIN `c`.`s`.`b` t
    ON s.`id` = t.`id`)
SELECT
...
,
    ...
    struct(
      ...
      FORMAT_NUMBER(target_nulls_v, 0) AS target_nulls,FORMAT_STRING('%.2f%%', CAST(
        CASE
          WHEN source_nulls_v = 0 THEN CASE WHEN target_nulls_v = 0 THEN 0.0 ELSE 100.0 END
          ELSE ABS(source_nulls_v - target_nulls_v) / CAST(source_nulls_v AS DOUBLE) * 100
        END AS DOUBLE
  ...
  CASE WHEN     CASE
      ...
      ELSE ABS(source_nulls_v - target_nulls_v) / CAST(source_nulls_v AS DOUBLE) <= 0.02
    END
 THEN 'PASS' ELSE 'FAIL' END AS status
    ) AS null_validation_v    ,
    struct(
      FORMAT_NUMBER(source_value_v_SUM, 2) as source_value,
      FORMAT_NUMBER(target_value_v_SUM, 2) as target_value,
...
:run_id, :job_id, 'job_name_here', :job_start_ts, validation_begin_ts, validation_complete_ts, source_catalog, source_schema, source_table, target_catalog, target_schema, target_table, business_domain, business_owner, business_priority, expected_sla_hours, estimated_impact_usd, result_payload FROM final_metrics_view;
