...
class TestDashboardQueryFixes:
    ...
    def test_dashboard_queries_are_correct(self):
        ...
        with patch('datapact.client.WorkspaceClient'):
            ...
            query = " ".join(validation_details["queryLines"]) if "queryLines" in validation_details else validation_details.get("query", "")
            
            # Should have proper status logic that checks all validation types
            assert "get_json_object(to_json(result_payload), '$.count_validation.status') = 'FAIL'" in query
            assert "get_json_object(to_json(result_payload), '$.row_hash_validation.status') = 'FAIL'" in query
            assert "to_json(result_payload) LIKE '%null_validation%status%FAIL%'" in query or "to_json(result_payload) LIKE '%null_validation%FAIL%'" in query
            assert "to_json(result_payload) LIKE '%agg_validation%FAIL%'" in query
            
            # Test 2: Source and target columns should handle nulls
            assert "CASE WHEN get_json_object(to_json(result_payload), '$.source_catalog') IS NOT NULL" in query
            assert "ELSE 'N/A' END as source_table" in query
            
            # Test 3: Agg validation check should use simplified logic
            assert "CASE WHEN to_json(result_payload) LIKE '%agg_validation%' AND to_json(result_payload) NOT LIKE '%agg_validation%FAIL%'" in query
            ...
            bi_query = " ".join(business_impact["queryLines"]) if "queryLines" in business_impact else business_impact.get("query", "")
            assert "COALESCE(get_json_object(to_json(result_payload), '$.source_schema'), 'Unknown Schema')" in bi_query
            ...
            ec_query = " ".join(exploded_checks["queryLines"]) if "queryLines" in exploded_checks else exploded_checks.get("query", "")
            # Should have single LATERAL VIEW without problematic split
            assert "LATERAL VIEW explode(array(" in ec_query
            assert "LATERAL VIEW explode(split(" not in ec_query
            ...
            lr_query = " ".join(latest_run["queryLines"]) if "queryLines" in latest_run else latest_run.get("query", "")
    ...
    def test_special_characters_in_job_names(self):
        ...
        with patch('datapact.client.WorkspaceClient'):
            ...
            for job_name in special_names:
                ...
                assert job_name.replace(' ', '_').replace(':', '') in display_name or job_name in display_name
    ...
    def test_all_validation_types_in_exploded_view(self):
        ...
        with patch('datapact.client.WorkspaceClient'):
            ...
            exploded_checks = next(d for d in datasets if d["name"] == "ds_exploded_checks")
            query = " ".join(exploded_checks["queryLines"]) if "queryLines" in exploded_checks else exploded_checks.get("query", "")
            ...
            # Should include the new amount_SUM aggregation
            assert "'amount_SUM'" in query
